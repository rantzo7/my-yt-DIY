...
export default function apiHandler (req, res, repo, connections = [], state = {}) {
  ...
  if (url.pathname === '/api/upload-cookies' && req.method === 'POST') { return uploadCookiesHandler(req, res) } // New endpoint
...
async function addChannelHandler (req, res, repo, connections = []) {
  ...
  try {
    ...
    if (Array.isArray(videos)) {
      ...
      res.writeHead(201)
      return res.end(`Channel "${name}" added successfully. Found ${videos.length} videos.`)
    } else {
      res.writeHead(404)
      return res.end(`Channel "${name}" not found or no videos could be fetched. Please check the channel name and try again.`)
    }
  } catch (error) {
    console.error(`Error in updateAndPersistVideosForChannel for channel "${name}":`, error)
    res.writeHead(500)
    return res.end(`An error occurred while fetching videos for channel "${name}": ${error.message || 'Unknown error'}. Please check server logs.`)
  }
}
...
async function uploadCookiesHandler (req, res) {
  try {
    const cookiesContent = await getBody(req)
    // Ensure the data directory exists
    const dataDir = './data'
    if (!fs.existsSync(dataDir)) {
      fs.mkdirSync(dataDir)
    }
    // Write the cookies.txt file to the project root
    fs.writeFileSync('./cookies.txt', cookiesContent)
    res.writeHead(200, { 'Content-Type': 'text/plain' })
    res.end('cookies.txt uploaded successfully')
  } catch (error) {
    console.error('Error uploading cookies.txt:', error)
    res.writeHead(500, { 'Content-Type': 'text/plain' })
    res.end(`Failed to upload cookies.txt: ${error.message}`)
  }
}
